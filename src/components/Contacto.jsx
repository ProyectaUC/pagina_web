...
import { Mail, Phone, MapPin, Send, Instagram, Facebook, Twitter, Linkedin, Youtube, CheckCircle } from 'lucide-react'
import { useInView } from '../hooks/useAnimations'
import { content, assets } from '../styles/theme/brand'

const socialLinks = [
  { icon: Instagram, href: content.social.instagram, label: 'Instagram', color: 'hover:text-pink-400'   },
  { icon: Facebook,  href: content.social.facebook,  label: 'Facebook',  color: 'hover:text-blue-400'   },
  { icon: Twitter,   href: content.social.twitter,   label: 'Twitter/X', color: 'hover:text-sky-400'    },
  { icon: Linkedin,  href: content.social.linkedin,  label: 'LinkedIn',  color: 'hover:text-blue-500'   },
  { icon: Youtube,   href: content.social.youtube,   label: 'YouTube',   color: 'hover:text-red-400'    },
]
...
export default function Contacto() {
  ...
  return (
    <>
      {/* ─── CONTACT SECTION ─── */}
      <section id="contacto" className="py-24 bg-proyecta-light-bg dark:bg-[#0D1F2A]" ref={ref}>
        <div className="section-container">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            >
              {/* Info cards */}
              {[
                { icon: Mail,   label: 'Email',     value: content.org.email,   href: `mailto:${content.org.email}` },
                { icon: Phone,  label: 'Teléfono',  value: content.org.phone,   href: `tel:${content.org.phone}`   },
                { icon: MapPin, label: 'Ubicación', value: content.org.address, href: null                          },
              ].map((item) => {
                ...
                const content_el = (
                  ...
                  >
                    <div className="p-3 rounded-xl bg-proyecta-cyan/10">
                      <Icon size={20} className="text-proyecta-cyan" strokeWidth={1.5} />
                    </div>
                    <div>
      ...
      <footer
        ...
        style={{ background: 'linear-gradient(160deg, #1B3A4B 0%, #1B5E7A 100%)' }}
      >
        <div className="section-container">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div className="lg:col-span-2">
              ...
              <p className="text-white/60 text-sm leading-relaxed max-w-sm">
                Voluntariado comprometido con el desarrollo comunitario, la educación y el bienestar
                social. Construyendo Chile desde la acción colectiva.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {['Quiénes Somos', 'Impacto', 'Programas', 'Apóyanos', 'Contacto'].map((item) => (
                  <li key={item}>
                    <a
          ...
          <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-white/30">
              © {new Date().getFullYear()} {content.org.fullName}. Todos los derechos reservados.
            </p>
