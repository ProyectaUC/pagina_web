...
function TransferRow({ label, value }) {
  ...
  const handleCopy = async () => {
    try {
      ...
      // Clipboard no disponible (permiso denegado, contexto no seguro,
      // etc.): el dato ya es visible en pantalla para copiar a mano.
    }
  };

  return (
    <div className="flex items-center justify-between gap-3 py-2.5 border-b border-gray-100 dark:border-white/10 last:border-0">
      <div className="min-w-0">
        <div className="text-xs text-gray-400 dark:text-white/40 uppercase tracking-wide">
          {label}
        </div>
        <div className="text-sm font-bold text-proyecta-navy dark:text-white truncate">
          {value}
        </div>
      </div>
      <button
        ...
        className="flex-shrink-0 min-w-9 min-h-9 flex items-center justify-center rounded-lg
                   text-gray-400 dark:text-white/40 hover:text-proyecta-teal dark:hover:text-proyecta-cyan
                   hover:bg-proyecta-cyan/10 transition-colors"
      >
        {copied ? (
    ...
    <div className="grid lg:grid-cols-2 gap-12 items-center">
      <div>
        ...
        <p className="text-gray-500 dark:text-white/60 mb-6 leading-relaxed">
          ...
          Como referencia, esto representa un aporte:
        </p>
      ...
      <div className="card dark:bg-proyecta-navy/60 dark:border-proyecta-teal/20 p-8 rounded-[2rem] shadow-xl bg-white border border-gray-100">
        ...
        <p className="text-xs text-gray-400 dark:text-white/40 text-center mt-5">
          Cualquier monto ayuda — gracias por tu aporte 💛
        </p>
      </div>
    </div>
  );
